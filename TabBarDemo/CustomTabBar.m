...
- (void)createTabBarView {
    ...
    for(int i=0;i<num;i++) {
        CustomTabBarButton *button = [[CustomTabBarButton alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/num*i, 0, [UIScreen mainScreen].bounds.size.width/num,self.tabBarHeight)];
        button.tag = 1000+i;
...
- (void)showControllerIndex:(NSInteger)index {
    ...
    if([self.customTabBarDelegate respondsToSelector:@selector(CustomTabBar:didSelectViewController:)]) {
        [self.customTabBarDelegate CustomTabBar:self didSelectViewController:self.viewControllers[index]];
    }
}
/**
 *  数字角标
